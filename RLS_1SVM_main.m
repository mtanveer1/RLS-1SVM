...
for run=1:tot_run    
    ...
    for ii = 1:length(C1)
        ...
        for iii = 1:length(C2)
            ...
            for t = 1:length(Sigma)
                ...
                %% training
                [alpha,rho,~]=RLS_1SVM_func(traindata,traindata,FunPara);
                [labeltr_OCLSSVM,theta_train] = RLS_1SVM_test_model1(traindata,traindata,alpha,rho,FunPara);
                labelval_OCLSSVM = RLS_1SVM_test_model2(valdata,traindata,alpha,rho,FunPara,theta_train);
    ...
    %% testing
    [alpha,rho,train_time]=RLS_1SVM_func(traindata,traindata,Funpara);
    label_OCLSSVM = RLS_1SVM_test_model3(testdata,traindata,alpha,rho,Funpara);
